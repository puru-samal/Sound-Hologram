...
- `3D1U_ST_Fixed reversals target_angle(optional) dist(optional)`

  - Runs the sequence of commands for a variant of the three_down_one_up experiment.
  - If target_angle and distance are unspecefied, source position will be set relative to
    the tracker. Ensure tracking is enabled!
  - Will prompt for ranges and step-sizes to support variable step-sizes.
    - Arguments:
      - reversals (int) : Number of reversals before termination
...
- `3D1U_FT_Fixed runs target_angle(optional) dist(optional)`
